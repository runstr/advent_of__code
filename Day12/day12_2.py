...
def check_small_cave_occurences(path):
    small_cave = []
    for cave in path:
        if cave.islower():
            small_cave.append(cave)
    return len(small_cave)==len(set(small_cave))
...
def check_path(cave, cave_mapping, paths, current_path, search):
    ...
    for new_cave in cave_mapping[cave]:
        if new_cave == "start":
            continue
        if (new_cave.islower() and new_cave in current_path) and not check_small_cave_occurences(new_current_path):
            continue
        else:
...
def execution():
    ...
    print("Answer to day {} task two is: {}".format(day, len(paths)))
